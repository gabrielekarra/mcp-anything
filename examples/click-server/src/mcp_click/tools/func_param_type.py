"""func_param_type tools for click."""
...
def register_tools(server: FastMCP, backend) -> None:
    """Register func_param_type tools with the server."""

    @server.tool()
    async def func_param_type_convert(
        value: str,
        param: str,
        ctx: str,
        init_func: str | None = None,
    ) -> str:
        """Convert"""
        # Instance method: types.FuncParamType.convert
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "FuncParamType")
        kwargs = {}
        kwargs["value"] = value
        kwargs["param"] = param
        kwargs["ctx"] = ctx
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            if init_func is not None:
                init_kwargs["func"] = init_func
            instance = _get_or_create_instance(mod, "FuncParamType", **init_kwargs)
    ...
    @server.tool()
    async def func_param_type_to_info_dict(
        init_func: str | None = None,
    ) -> str:
        """To info dict"""
        # Instance method: types.FuncParamType.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "FuncParamType")
        kwargs = {}
