...
def register_tools(server: FastMCP, backend) -> None:
    """Register lazy_file tools with the server."""

    @server.tool()
    async def lazy_file_close(
        init_filename: str | None = None,
    ) -> str:
        """Closes the underlying file, no matter what."""
        # Instance method: utils.LazyFile.close
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "utils")
        cls = getattr(mod, "LazyFile")
        kwargs = {}
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            if init_filename is not None:
                init_kwargs["filename"] = init_filename
            instance = _get_or_create_instance(mod, "LazyFile", **init_kwargs)
    ...
    @server.tool()
    async def lazy_file_close_intelligently(
        init_filename: str | None = None,
    ) -> str:
        """This function only closes the file if it was opened by the lazy"""
        # Instance method: utils.LazyFile.close_intelligently
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "utils")
        cls = getattr(mod, "LazyFile")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "close_intelligently")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "close_intelligently")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_filename is not None:
                init_kwargs["filename"] = init_filename
            instance = _get_or_create_instance(mod, "LazyFile", **init_kwargs)
            result = getattr(instance, "close_intelligently")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def lazy_file_open(
        init_filename: str | None = None,
    ) -> str:
        """Opens the file if it's not yet open.  This call might fail with"""
        # Instance method: utils.LazyFile.open
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "utils")
        cls = getattr(mod, "LazyFile")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "open")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "open")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_filename is not None:
                init_kwargs["filename"] = init_filename
            instance = _get_or_create_instance(mod, "LazyFile", **init_kwargs)
            result = getattr(instance, "open")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
