...
def register_tools(server: FastMCP, backend) -> None:
    """Register progress_bar tools with the server."""

    @server.tool()
    async def progress_bar_eta(
        init_iterable: str | None = None,
    ) -> str:
        """Eta"""
        # Instance method: _termui_impl.ProgressBar.eta
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "ProgressBar")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "eta")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "eta")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_iterable is not None:
                init_kwargs["iterable"] = init_iterable
            instance = _get_or_create_instance(mod, "ProgressBar", **init_kwargs)
            result = getattr(instance, "eta")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def progress_bar_generator(
        init_iterable: str | None = None,
    ) -> str:
        """Return a generator which yields the items added to the bar"""
        # Instance method: _termui_impl.ProgressBar.generator
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "ProgressBar")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "generator")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "generator")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_iterable is not None:
                init_kwargs["iterable"] = init_iterable
            instance = _get_or_create_instance(mod, "ProgressBar", **init_kwargs)
            result = getattr(instance, "generator")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def progress_bar_make_step(
        n_steps: int,
        init_iterable: str | None = None,
    ) -> str:
        """Make step"""
        # Instance method: _termui_impl.ProgressBar.make_step
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "ProgressBar")
        kwargs = {}
        kwargs["n_steps"] = n_steps
        method_attr = inspect.getattr_static(cls, "make_step")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "make_step")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_iterable is not None:
                init_kwargs["iterable"] = init_iterable
            instance = _get_or_create_instance(mod, "ProgressBar", **init_kwargs)
            result = getattr(instance, "make_step")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def progress_bar_pct(
        init_iterable: str | None = None,
    ) -> str:
        """Pct"""
        # Instance method: _termui_impl.ProgressBar.pct
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "ProgressBar")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "pct")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "pct")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_iterable is not None:
                init_kwargs["iterable"] = init_iterable
            instance = _get_or_create_instance(mod, "ProgressBar", **init_kwargs)
            result = getattr(instance, "pct")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def progress_bar_time_per_iteration(
        init_iterable: str | None = None,
    ) -> str:
        """Time per iteration"""
        # Instance method: _termui_impl.ProgressBar.time_per_iteration
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "ProgressBar")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "time_per_iteration")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "time_per_iteration")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_iterable is not None:
                init_kwargs["iterable"] = init_iterable
            instance = _get_or_create_instance(mod, "ProgressBar", **init_kwargs)
            result = getattr(instance, "time_per_iteration")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
