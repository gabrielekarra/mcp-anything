...
def register_tools(server: FastMCP, backend) -> None:
    """Register date_time tools with the server."""

    @server.tool()
    async def date_time_convert(
        value: str,
        param: str,
        ctx: str,
    ) -> str:
        """Convert"""
        # Instance method: types.DateTime.convert
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "DateTime")
        kwargs = {}
        kwargs["value"] = value
        kwargs["param"] = param
        kwargs["ctx"] = ctx
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "DateTime", **init_kwargs)
    ...
    @server.tool()
    async def date_time_get_metavar(
        param: str,
        ctx: str,
    ) -> str:
        """Get metavar"""
        # Instance method: types.DateTime.get_metavar
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "DateTime")
        kwargs = {}
    ...
    @server.tool()
    async def date_time_to_info_dict(
    ) -> str:
        """To info dict"""
        # Instance method: types.DateTime.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "DateTime")
        kwargs = {}
