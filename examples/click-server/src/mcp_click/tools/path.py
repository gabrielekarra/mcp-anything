...
def register_tools(server: FastMCP, backend) -> None:
    """Register path tools with the server."""

    @server.tool()
    async def path_coerce_path_result(
        value: str,
    ) -> str:
        """Coerce path result"""
        # Instance method: types.Path.coerce_path_result
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Path")
        kwargs = {}
        kwargs["value"] = value
        method_attr = inspect.getattr_static(cls, "coerce_path_result")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "coerce_path_result")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Path", **init_kwargs)
            result = getattr(instance, "coerce_path_result")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def path_convert(
        value: str,
        param: str,
        ctx: str,
    ) -> str:
        """Convert"""
        # Instance method: types.Path.convert
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Path")
        kwargs = {}
    ...
    @server.tool()
    async def path_shell_complete(
        ctx: str,
        param: str,
        incomplete: str,
    ) -> str:
        """Return a special completion marker that tells the completion"""
        # Instance method: types.Path.shell_complete
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Path")
        kwargs = {}
    ...
    @server.tool()
    async def path_to_info_dict(
    ) -> str:
        """To info dict"""
        # Instance method: types.Path.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Path")
        kwargs = {}
