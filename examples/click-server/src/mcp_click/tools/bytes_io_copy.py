...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def bytes_io_copy_write(
        b: str,
        init_copy_to: str | None = None,
    ) -> str:
        """Write"""
        # Instance method: testing.BytesIOCopy.write
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "BytesIOCopy")
        kwargs = {}
        kwargs["b"] = b
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            init_kwargs = {}
            if init_copy_to is not None:
                init_kwargs["copy_to"] = init_copy_to
            instance = _get_or_create_instance(mod, "BytesIOCopy", **init_kwargs)
