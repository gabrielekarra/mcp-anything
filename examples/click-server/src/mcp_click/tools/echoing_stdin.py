...
def register_tools(server: FastMCP, backend) -> None:
    """Register echoing_stdin tools with the server."""

    @server.tool()
    async def echoing_stdin_read(
        n: int | None = None,
        init_input: str | None = None,
        init_output: str | None = None,
    ) -> str:
        """Read"""
        # Instance method: testing.EchoingStdin.read
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "EchoingStdin")
        kwargs = {}
        if n is not None:
            kwargs["n"] = n
        method_attr = inspect.getattr_static(cls, "read")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "read")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_input is not None:
                init_kwargs["input"] = init_input
            if init_output is not None:
                init_kwargs["output"] = init_output
            instance = _get_or_create_instance(mod, "EchoingStdin", **init_kwargs)
            result = getattr(instance, "read")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def echoing_stdin_read1(
        n: int | None = None,
        init_input: str | None = None,
        init_output: str | None = None,
    ) -> str:
        """Read1"""
        # Instance method: testing.EchoingStdin.read1
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "EchoingStdin")
        kwargs = {}
        if n is not None:
            kwargs["n"] = n
        method_attr = inspect.getattr_static(cls, "read1")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "read1")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_input is not None:
            ...
            if init_output is not None:
                init_kwargs["output"] = init_output
            instance = _get_or_create_instance(mod, "EchoingStdin", **init_kwargs)
            result = getattr(instance, "read1")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def echoing_stdin_readline(
        n: int | None = None,
        init_input: str | None = None,
        init_output: str | None = None,
    ) -> str:
        """Readline"""
        # Instance method: testing.EchoingStdin.readline
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "EchoingStdin")
        kwargs = {}
        if n is not None:
            kwargs["n"] = n
        method_attr = inspect.getattr_static(cls, "readline")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "readline")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_input is not None:
            ...
            if init_output is not None:
                init_kwargs["output"] = init_output
            instance = _get_or_create_instance(mod, "EchoingStdin", **init_kwargs)
            result = getattr(instance, "readline")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
