...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def cli_runner_get_default_prog_name(
        ...
        # Instance method: testing.CliRunner.get_default_prog_name
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "CliRunner")
        kwargs = {}
        kwargs["cli"] = cli
        method_attr = inspect.getattr_static(cls, "get_default_prog_name")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "get_default_prog_name")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "CliRunner", **init_kwargs)
            result = getattr(instance, "get_default_prog_name")(**kwargs)
    ...
    async def cli_runner_isolated_filesystem(
        ...
        # Instance method: testing.CliRunner.isolated_filesystem
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "CliRunner")
        kwargs = {}
        ...
        method_attr = inspect.getattr_static(cls, "isolated_filesystem")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "isolated_filesystem")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "CliRunner", **init_kwargs)
            result = getattr(instance, "isolated_filesystem")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
    ...
    async def cli_runner_make_env(
        ...
        # Instance method: testing.CliRunner.make_env
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "testing")
        cls = getattr(mod, "CliRunner")
        kwargs = {}
        ...
        method_attr = inspect.getattr_static(cls, "make_env")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "make_env")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "CliRunner", **init_kwargs)
            result = getattr(instance, "make_env")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
