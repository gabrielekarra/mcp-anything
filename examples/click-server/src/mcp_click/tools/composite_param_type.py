...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def composite_param_type_arity(
        ...
        """Arity"""
        # Instance method: types.CompositeParamType.arity
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "CompositeParamType")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "arity")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "arity")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "CompositeParamType", **init_kwargs)
            result = getattr(instance, "arity")(**kwargs)
