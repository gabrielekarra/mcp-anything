"""no_args_is_help_error tools for click."""
...
def register_tools(server: FastMCP, backend) -> None:
    """Register no_args_is_help_error tools with the server."""

    @server.tool()
    async def no_args_is_help_error_show(
        file: str | None = None,
        init_ctx: str | None = None,
    ) -> str:
        """Show"""
        # Instance method: exceptions.NoArgsIsHelpError.show
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "exceptions")
        cls = getattr(mod, "NoArgsIsHelpError")
        kwargs = {}
        if file is not None:
            kwargs["file"] = file
        method_attr = inspect.getattr_static(cls, "show")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "show")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_ctx is not None:
                init_kwargs["ctx"] = init_ctx
            instance = _get_or_create_instance(mod, "NoArgsIsHelpError", **init_kwargs)
            result = getattr(instance, "show")(**kwargs)
