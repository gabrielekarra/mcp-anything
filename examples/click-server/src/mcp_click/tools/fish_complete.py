...
def register_tools(server: FastMCP, backend) -> None:
    """Register fish_complete tools with the server."""

    @server.tool()
    async def fish_complete_format_completion(
        item: str,
    ) -> str:
        """Format completion"""
        # Instance method: shell_completion.FishComplete.format_completion
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "shell_completion")
        cls = getattr(mod, "FishComplete")
        kwargs = {}
        kwargs["item"] = item
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "FishComplete", **init_kwargs)
