...
def register_tools(server: FastMCP, backend) -> None:
    """Register editor tools with the server."""
    ...
    async def editor_edit(
        ...
        """Edit"""
        # Instance method: _termui_impl.Editor.edit
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "Editor")
        kwargs = {}
        kwargs["text"] = text
        method_attr = inspect.getattr_static(cls, "edit")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "edit")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Editor", **init_kwargs)
            result = getattr(instance, "edit")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def editor_edit_files(
        filenames: str,
    ) -> str:
        """Edit files"""
        # Instance method: _termui_impl.Editor.edit_files
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_termui_impl")
        cls = getattr(mod, "Editor")
        kwargs = {}
        kwargs["filenames"] = filenames
        method_attr = inspect.getattr_static(cls, "edit_files")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "edit_files")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Editor", **init_kwargs)
            result = getattr(instance, "edit_files")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
