...
def register_tools(server: FastMCP, backend) -> None:
    """Register param_type tools with the server."""

    @server.tool()
    async def param_type_convert(
        value: str,
        param: str,
        ctx: str,
    ) -> str:
        """Convert the value to the correct type. This is not called if"""
        # Instance method: types.ParamType.convert
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
        kwargs["value"] = value
        kwargs["param"] = param
        kwargs["ctx"] = ctx
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "ParamType", **init_kwargs)
    ...
    @server.tool()
    async def param_type_fail(
        message: str,
        param: str | None = None,
        ctx: str | None = None,
    ) -> str:
        """Helper method to fail with an invalid value message."""
        # Instance method: types.ParamType.fail
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
    ...
    @server.tool()
    async def param_type_get_metavar(
        param: str,
        ctx: str,
    ) -> str:
        """Returns the metavar default for this param if it provides one."""
        # Instance method: types.ParamType.get_metavar
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
        kwargs["param"] = param
        kwargs["ctx"] = ctx
        method_attr = inspect.getattr_static(cls, "get_metavar")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "get_metavar")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "ParamType", **init_kwargs)
            result = getattr(instance, "get_metavar")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def param_type_get_missing_message(
        param: str,
        ctx: str,
    ) -> str:
        """Optionally might return extra information about a missing"""
        # Instance method: types.ParamType.get_missing_message
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
        kwargs["param"] = param
        kwargs["ctx"] = ctx
        method_attr = inspect.getattr_static(cls, "get_missing_message")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "get_missing_message")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "ParamType", **init_kwargs)
            result = getattr(instance, "get_missing_message")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def param_type_shell_complete(
        ctx: str,
        param: str,
        incomplete: str,
    ) -> str:
        """Return a list of"""
        # Instance method: types.ParamType.shell_complete
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
    ...
    @server.tool()
    async def param_type_split_envvar_value(
        rv: str,
    ) -> str:
        """Given a value from an environment variable this splits it up"""
        # Instance method: types.ParamType.split_envvar_value
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
        kwargs["rv"] = rv
        method_attr = inspect.getattr_static(cls, "split_envvar_value")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "split_envvar_value")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "ParamType", **init_kwargs)
            result = getattr(instance, "split_envvar_value")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def param_type_to_info_dict(
    ) -> str:
        """Gather information that could be useful for a tool generating"""
        # Instance method: types.ParamType.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "ParamType")
        kwargs = {}
