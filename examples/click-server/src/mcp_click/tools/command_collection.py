...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def command_collection_add_source(
        ...
        # Instance method: core.CommandCollection.add_source
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "CommandCollection")
        kwargs = {}
        kwargs["group"] = group
        method_attr = inspect.getattr_static(cls, "add_source")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "add_source")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "CommandCollection", **init_kwargs)
            result = getattr(instance, "add_source")(**kwargs)
    ...
    async def command_collection_get_command(
        ...
        """Get command"""
        # Instance method: core.CommandCollection.get_command
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "CommandCollection")
        kwargs = {}
    ...
    async def command_collection_list_commands(
        ...
        """List commands"""
        # Instance method: core.CommandCollection.list_commands
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "CommandCollection")
        kwargs = {}
