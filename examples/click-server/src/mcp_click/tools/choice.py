...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def choice_convert(
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            init_kwargs = {}
            if init_choices is not None:
                init_kwargs["choices"] = init_choices
            instance = _get_or_create_instance(mod, "Choice", **init_kwargs)
    ...
    async def choice_get_invalid_choice_message(
        ...
        # Instance method: types.Choice.get_invalid_choice_message
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Choice")
        kwargs = {}
        kwargs["value"] = value
        kwargs["ctx"] = ctx
        method_attr = inspect.getattr_static(cls, "get_invalid_choice_message")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "get_invalid_choice_message")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_choices is not None:
                init_kwargs["choices"] = init_choices
            instance = _get_or_create_instance(mod, "Choice", **init_kwargs)
            result = getattr(instance, "get_invalid_choice_message")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
    ...
    async def choice_get_metavar(
        ...
        """Get metavar"""
        # Instance method: types.Choice.get_metavar
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Choice")
        kwargs = {}
    ...
    async def choice_get_missing_message(
        ...
        # Instance method: types.Choice.get_missing_message
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Choice")
        kwargs = {}
    ...
    async def choice_normalize_choice(
        ...
        # Instance method: types.Choice.normalize_choice
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Choice")
        kwargs = {}
        ...
        kwargs["ctx"] = ctx
        method_attr = inspect.getattr_static(cls, "normalize_choice")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "normalize_choice")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_choices is not None:
                init_kwargs["choices"] = init_choices
            instance = _get_or_create_instance(mod, "Choice", **init_kwargs)
            result = getattr(instance, "normalize_choice")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
    ...
    async def choice_shell_complete(
        ...
        # Instance method: types.Choice.shell_complete
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Choice")
        kwargs = {}
    ...
    async def choice_to_info_dict(
        ...
        """To info dict"""
        # Instance method: types.Choice.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "Choice")
        kwargs = {}
