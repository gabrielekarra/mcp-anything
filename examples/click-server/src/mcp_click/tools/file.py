...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def file_convert(
        ...
        if isinstance(method_attr, (staticmethod, classmethod)):
            ...
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "File", **init_kwargs)
    ...
    @server.tool()
    async def file_resolve_lazy_flag(
        value: str,
    ) -> str:
        """Resolve lazy flag"""
        # Instance method: types.File.resolve_lazy_flag
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "File")
        kwargs = {}
        kwargs["value"] = value
        method_attr = inspect.getattr_static(cls, "resolve_lazy_flag")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "resolve_lazy_flag")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "File", **init_kwargs)
            result = getattr(instance, "resolve_lazy_flag")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def file_shell_complete(
        ctx: str,
        param: str,
        incomplete: str,
    ) -> str:
        """Return a special completion marker that tells the completion"""
        # Instance method: types.File.shell_complete
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "File")
        kwargs = {}
    ...
    @server.tool()
    async def file_to_info_dict(
    ) -> str:
        """To info dict"""
        # Instance method: types.File.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "types")
        cls = getattr(mod, "File")
        kwargs = {}
