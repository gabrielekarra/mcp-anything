...
def register_tools(server: FastMCP, backend) -> None:
    """Register group tools with the server."""

    @server.tool()
    async def group_add_command(
        cmd: str,
        name: str | None = None,
    ) -> str:
        """Registers another :class:`Command` with this group.  If the name"""
        # Instance method: core.Group.add_command
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        kwargs["cmd"] = cmd
        if name is not None:
            kwargs["name"] = name
        method_attr = inspect.getattr_static(cls, "add_command")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "add_command")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "add_command")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def group_collect_usage_pieces(
        ctx: str,
    ) -> str:
        """Collect usage pieces"""
        # Instance method: core.Group.collect_usage_pieces
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
    ...
    async def group_command(
        ...
        """Command"""
        # Instance method: core.Group.command
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        kwargs["__func"] = func
        method_attr = inspect.getattr_static(cls, "command")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "command")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "command")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def group_format_commands(
        ctx: str,
        formatter: str,
    ) -> str:
        """Extra format methods for multi methods that adds all the commands"""
        # Instance method: core.Group.format_commands
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        ...
        method_attr = inspect.getattr_static(cls, "format_commands")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "format_commands")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "format_commands")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def group_format_options(
        ctx: str,
        formatter: str,
    ) -> str:
        """Format options"""
        # Instance method: core.Group.format_options
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
    ...
    @server.tool()
    async def group_get_command(
        ctx: str,
        cmd_name: str,
    ) -> str:
        """Given a context and a command name, this returns a :class:`Command`"""
        # Instance method: core.Group.get_command
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        ...
        method_attr = inspect.getattr_static(cls, "get_command")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "get_command")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "get_command")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
    ...
    async def group_group(
        ...
        """Group"""
        # Instance method: core.Group.group
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        kwargs["__func"] = func
        method_attr = inspect.getattr_static(cls, "group")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "group")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "group")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def group_list_commands(
        ctx: str,
    ) -> str:
        """Returns a list of subcommand names in the order they should appear."""
        # Instance method: core.Group.list_commands
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        kwargs["ctx"] = ctx
        method_attr = inspect.getattr_static(cls, "list_commands")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "list_commands")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "list_commands")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def group_resolve_command(
        ctx: str,
        args: list,
    ) -> str:
        """Resolve command"""
        # Instance method: core.Group.resolve_command
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
        kwargs["ctx"] = ctx
        kwargs["args"] = args
        method_attr = inspect.getattr_static(cls, "resolve_command")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "resolve_command")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            instance = _get_or_create_instance(mod, "Group", **init_kwargs)
            result = getattr(instance, "resolve_command")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"

    @server.tool()
    async def group_shell_complete(
        ctx: str,
        incomplete: str,
    ) -> str:
        """Return a list of completions for the incomplete value. Looks"""
        # Instance method: core.Group.shell_complete
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
    ...
    @server.tool()
    async def group_to_info_dict(
        ctx: str,
    ) -> str:
        """To info dict"""
        # Instance method: core.Group.to_info_dict
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "core")
        cls = getattr(mod, "Group")
        kwargs = {}
