...
def register_tools(server: FastMCP, backend) -> None:
    ...
    async def console_stream_name(
        init_text_stream: str | None = None,
        init_byte_stream: str | None = None,
    ) -> str:
        """Name"""
        # Instance method: _winconsole.ConsoleStream.name
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_winconsole")
        cls = getattr(mod, "ConsoleStream")
        kwargs = {}
        method_attr = inspect.getattr_static(cls, "name")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "name")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_text_stream is not None:
                init_kwargs["text_stream"] = init_text_stream
            if init_byte_stream is not None:
                init_kwargs["byte_stream"] = init_byte_stream
            instance = _get_or_create_instance(mod, "ConsoleStream", **init_kwargs)
            result = getattr(instance, "name")(**kwargs)
    ...
    async def console_stream_write(
        ...
        """Write"""
        # Instance method: _winconsole.ConsoleStream.write
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_winconsole")
        cls = getattr(mod, "ConsoleStream")
        kwargs = {}
    ...
    async def console_stream_writelines(
        ...
        """Writelines"""
        # Instance method: _winconsole.ConsoleStream.writelines
        _setup_import_path("/tmp/click/src/click")
        mod = _load_source_module("/tmp/click/src/click", "_winconsole")
        cls = getattr(mod, "ConsoleStream")
        kwargs = {}
        kwargs["lines"] = lines
        method_attr = inspect.getattr_static(cls, "writelines")
        if isinstance(method_attr, (staticmethod, classmethod)):
            # Static/class method — call directly on the class
            result = getattr(cls, "writelines")(**kwargs)
        else:
            # Instance method — auto-instantiate the class
            init_kwargs = {}
            if init_text_stream is not None:
            ...
            if init_byte_stream is not None:
                init_kwargs["byte_stream"] = init_byte_stream
            instance = _get_or_create_instance(mod, "ConsoleStream", **init_kwargs)
            result = getattr(instance, "writelines")(**kwargs)
        if inspect.isawaitable(result):
            result = await result
        return str(result) if result is not None else "OK"
