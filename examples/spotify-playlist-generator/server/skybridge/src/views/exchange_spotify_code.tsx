...
function ExchangeSpotifyCodeView() {
  const { status, data, error, callTool } = useCallTool<ExchangeCodeArgs>("exchange_spotify_code");
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#121212", color: "#fff", padding: 28, fontFamily: "Inter, system-ui, sans-serif" }}>
      <h1 style={{ margin: 0, fontSize: 32, letterSpacing: 0 }}>Exchange Spotify Code</h1>
      <div style={{ display: "grid", gap: 10, maxWidth: 760, marginTop: 24 }}>
        <label style={{ color: "#b3b3b3", fontSize: 13, fontWeight: 700 }}>Callback code</label>
        <input
          value={code}
        ...
        />
        <button
          onClick={() => callTool({ code, redirect_uri: redirectUri })}
      ...
      {payload?.refresh_token && (
        <pre style={{ marginTop: 24, background: "#181818", color: "#1db954", padding: 16, overflowX: "auto" }}>
...
export default ExchangeSpotifyCodeView;
mountView(<ExchangeSpotifyCodeView />);
